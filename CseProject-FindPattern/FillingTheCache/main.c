...
int mapping_the_cache(){
	...
	/*Lines_Pool lines_pool;
	printf("set1 = %d, %p\n",4000,EvictionSets.Sets[4000].Lines[0]);
	int size_pool = find_lines_pool(&EvictionSets.Sets[4000],Buffer.Lines->Bytes, sizeof(LinesBuffer),&lines_pool);
	printf("size pool = %d, val lines = %d\n",size_pool,lines_pool.ValidLines);
	create_multiple_list(&lines_pool);
	printf("original list\n");
	printf("set2 = %d, %p\n",4000,EvictionSets.Sets[4000].Lines[0]);
	print_list(&EvictionSets.Sets[4000]);
	printf("done here\n");*/
	return SetsFound;
}
...
int Extract_key_From_Set(int set){
	...
	/*char *pEvictionLine = find_eviction_line(&EvictionSets.Sets[set],Buffer.Lines->Bytes, sizeof(LinesBuffer));
	char* pKey = ExtractKeyFromSetWithSelfNoise(&EvictionSets.Sets[set],pEvictionLine, OnesPattern, ZerosPattern, PATTERN_LEN, numOfSamples,RsaKeys[0],&samples_found);*/
	
	//using pool:
	/*Lines_Pool lines_pool;
	int size_pool = find_lines_pool(&EvictionSets.Sets[set],Buffer.Lines->Bytes, sizeof(LinesBuffer),&lines_pool);
	
	create_multiple_list(&lines_pool);
	
	print_list(&EvictionSets.Sets[set]);
	
	char* pKey = ExtractKeyFromPool(&lines_pool, OnesPattern, ZerosPattern, PATTERN_LEN, numOfSamples,RsaKeys[0],&samples_found);*/
